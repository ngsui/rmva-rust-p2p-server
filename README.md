...
**游戏内登录系统**（按 L 唤出）——Win32 原生登录面板，账号数据经 argon2id 哈希存于 PostgreSQL，登录后签发会话 token：

![游戏登录系统](screenshots/游戏登录系统截图.png)

**游戏内拍卖行**（按 P 唤出）——三页式交易界面（市场 / 我的挂单 / 上架），金币与物品结算全部在服务器端事务内完成，客户端无法伪造余额：

![游戏拍卖行页面](screenshots/游戏拍卖行页面截图.png)

**云端生产部署**——腾讯云轻量服务器（Debian 12）上 systemd 守护运行，journald 实时日志可见玩家连接、注册、拍卖行操作全链路：

![腾讯云控制台](screenshots/腾讯云的控制台.png)
