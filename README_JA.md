...
**ゲーム内ログインシステム**（Lキーで起動）——ネイティブWin32ログインパネル。アカウントはargon2idハッシュでPostgreSQLに保存され、ログイン後にセッショントークンを発行：

![ログインシステム](screenshots/游戏登录系统截图.png)

**ゲーム内オークションハウス**（Pキーで起動）——3タブ構成の取引UI（市場 / 自分の出品 / 出品）。ゴールドとアイテムの決済はすべてサーバー側トランザクション内で行われ、クライアントは残高を偽造できない：

![オークションハウス](screenshots/游戏拍卖行页面截图.png)

**クラウド本番運用**——Tencent Cloud軽量アプリケーションサーバー（Debian 12）上でsystemdサービスとして稼働。journaldのログにはプレイヤー接続・登録・オークション操作の全リンクが表示される：

![Tencent Cloudコンソール](screenshots/腾讯云的控制台.png)
